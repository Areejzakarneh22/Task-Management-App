...
namespace Task_Management__App
{
    public class Filter
    {
            ...
            try
            {
                ...
                taskList = xmlDoc.Descendants("Task")
                                 .Select(task => new TaskBasics
                                 {
                                     ...
                                     Category = task.Element("Category").Value,
                                 })
                                 .ToList();
            }
            catch (Exception ex)
        ...
        {
            Console.WriteLine("Select filter option: 1. Department 2. Title 3. Assignee 4. category");
        ...
        public void CategoryFilter()
        {
            ...
            var filteredResults = from task in taskList
                                  where task.Category.Equals(category, StringComparison.OrdinalIgnoreCase)
                                  select $"{task.Id}- {task.Title} assigned to {task.Assignee} in department {task.Department}";
